...
func TestMergeForciblyStop(t *testing.T) {
	...
	dmis := &uint64set.Set{}
	if err := mergeBlockStreams(nil, &mp.ph, &bsw, bsrs, ch, dmis, 0, &rowsMerged, &rowsDeleted, true, partInmemory); !errors.Is(err, errForciblyStopped) {
...
func testMergeBlockStreams(t *testing.T, bsrs []*blockStreamReader, expectedBlocksCount, expectedRowsCount int, expectedMinTimestamp, expectedMaxTimestamp int64) {
	...
	dmis := &uint64set.Set{}
	var rowsMerged, rowsDeleted atomic.Uint64
	if err := mergeBlockStreams(nil, &mp.ph, &bsw, bsrs, nil, dmis, 0, &rowsMerged, &rowsDeleted, true, partInmemory); err != nil {
